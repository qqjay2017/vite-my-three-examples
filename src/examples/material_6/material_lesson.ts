...
export class MaterialLessonInstance extends ThreeInstanceBase {
  ...
  material: THREE.MeshToonMaterial | null = null;
  ...
  gui() {
    ...
    materialFolder
      .add(this.material, "gradientMap", ["none", "three", "five"])
      .onChange((val) => {
        if (!this.material) {
          return;
        }
        if (val == "none") {
          ...
          this.material.gradientMap = this.threeToneTexture;
        } else if (val === "five") {
          this.material.gradientMap = this.fiveToneTexture;
        }
        this.material.needsUpdate = true;
      });
